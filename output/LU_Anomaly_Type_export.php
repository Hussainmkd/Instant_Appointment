...
$layout->blocks["top"][] = "export";$page_layouts["LU_Anomaly_Type_export"] = $layout;
...
$xt->display("LU_Anomaly_Type_export.htm");

function ExportToWord()
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-word");
	header("Content-Disposition: attachment;Filename=LU_Anomaly_Type.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=LU_Anomaly_Type.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$values["Tamper Type"] = GetData($row,"Tamper Type","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=LU_Anomaly_Type.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["Tamper Type"] = GetData($row,"Tamper Type",$format);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"','""',$values["Tamper Type"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_LU_Anomaly_Type","Code")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_LU_Anomaly_Type","Tamper_Type")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("dbo_LU_Anomaly_Type","Code")."</td>";
		echo "<td>".GetFieldLabel("dbo_LU_Anomaly_Type","Tamper_Type")."</td>";
	}
	echo "</tr>";
		$totals["Code"]=0;
		$totalsFields[]=array('fName'=>"Code", 'totalsType'=>'', 'viewFormat'=>"");
		$totals["Tamper Type"]=0;
		$totalsFields[]=array('fName'=>"Tamper Type", 'totalsType'=>'', 'viewFormat'=>"");
	$totals=array();
// write data rows
	$iNumberOfRows = 0;
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Tamper Type"]);
					else
						echo htmlspecialchars($values["Tamper Type"]);
			echo '</td>';
			echo "</tr>";
		}		
