...
$layout->blocks["top"][] = "details";$page_layouts["LU_Module_Condition_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "LU_Module_Condition_inline_add.htm";
else
	$templatefile = "LU_Module_Condition_add.htm";
...
//	processing Condition - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["Condition"]=$value;
		}
		}
//	processibng Condition - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'LU_Module_Condition_edit.php?'.$keylink.'\'>'.mlang_message("EDIT").'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'LU_Module_Condition_view.php?'.$keylink.'\'>'.mlang_message("VIEW").'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: LU_Module_Condition_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["Condition"]=@$avalues["Condition"];
}
