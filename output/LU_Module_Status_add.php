...
$layout->blocks["top"][] = "details";$page_layouts["LU_Module_Status_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "LU_Module_Status_inline_add.htm";
else
	$templatefile = "LU_Module_Status_add.htm";
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: LU_Module_Status_".$pageObject->getPageType().".php");
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.LU_Module Status";
		$options['firstTime'] = 1;
