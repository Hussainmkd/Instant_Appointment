...
$layout->blocks["top"][] = "details";$page_layouts["System_Users_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "System_Users_inline_edit.htm" : "System_Users_edit.htm";
...
//	processing UserID - begin
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "UserID"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
...
//	processing User Name - begin
	$condition = 1;

	if($condition)
	{
		$value = postvalue("value_User_Name_".$id);
		$type = postvalue("type_User_Name_".$id);
		if(FieldSubmitted("User Name_".$id))
		{
				$value = prepare_for_db("User Name",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "User Name"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["User Name"] = $value;
		
			}
	
		}
//	processing User Name - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: System_Users_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_System_Users->Copy();
...
if(!$data)
{
	...
	{
		header("Location: System_Users_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["User Name"] = $evalues["User Name"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"UserID", "")));
...
//	User Name - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"User Name", ""),"field=User+Name".$keylink,"",MODE_LIST);
	$showValues["User Name"] = $value;
	$showFields[] = "User Name";
		$showRawValues["User Name"] = substr($data["User Name"],0,100);

//	Role - 

		$value = "";
				$value = DisplayLookupWizard("Role",$data["Role"],$data,$keylink,MODE_LIST);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.System Users";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='System_Users_view.php?".$viewlink."'\"");
