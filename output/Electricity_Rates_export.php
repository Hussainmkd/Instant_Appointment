...
$layout->blocks["top"][] = "export";$page_layouts["Electricity_Rates_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["Units"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$xt->display("Electricity_Rates_export.htm");

function ExportToWord()
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-word");
	header("Content-Disposition: attachment;Filename=Electricity_Rates.doc");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Electricity_Rates.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["Units"] = GetData($row,"Units",$format);
			$format="Number";
			$values["PerUnit Price"] = $row["PerUnit Price"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"','""',$values["Units"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"','""',$values["PerUnit Price"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_Electricity_Rates","Units")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_Electricity_Rates","PerUnit_Price")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("dbo_Electricity_Rates","Units")."</td>";
		echo "<td>".GetFieldLabel("dbo_Electricity_Rates","PerUnit_Price")."</td>";
	}
	echo "</tr>";
		$totals["Units"]=0;
		$totalsFields[]=array('fName'=>"Units", 'totalsType'=>'', 'viewFormat'=>"");
		$totals["PerUnit Price"]=0;
		$totalsFields[]=array('fName'=>"PerUnit Price", 'totalsType'=>'', 'viewFormat'=>"Number");
	$totals=array();
// write data rows
	$iNumberOfRows = 0;
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["PerUnit Price"] = GetData($row,"PerUnit Price",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["Units"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="Number";
									echo htmlspecialchars($values["PerUnit Price"]);
			echo '</td>';
			echo "</tr>";
		}		
