...
$layout->blocks["top"][] = "details";$page_layouts["Module_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Module_inline_edit.htm" : "Module_edit.htm";
...
//	processing Module Status - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Module Status"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Module Status"] = $value;
		
			}
	
		}
//	processing Module Status - end
//	processing Module Condition - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Module Condition"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Module Condition"] = $value;
		
			}
	
		}
//	processing Module Condition - end
//	processing Serial Num - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Serial Num"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Serial Num"] = $value;
		
			}
	
		}
//	processing Serial Num - end
//	processing Entry Date - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Entry Date"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Entry Date"] = $value;
		
			}
	
		}
//	processing Entry Date - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Module_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Module_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Module Type"] = $evalues["Module Type"];
	$data["Module Status"] = $evalues["Module Status"];
	$data["Module Condition"] = $evalues["Module Condition"];
	$data["Serial Num"] = $evalues["Serial Num"];
	$data["Entry Date"] = $evalues["Entry Date"];
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Module_view.php?".$viewlink."'\"");
