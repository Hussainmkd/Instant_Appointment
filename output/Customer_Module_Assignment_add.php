...
$layout->blocks["top"][] = "details";$page_layouts["Customer_Module_Assignment_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Customer_Module_Assignment_inline_add.htm";
else
	$templatefile = "Customer_Module_Assignment_add.htm";
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="dbo.Customers")
	{
		...
		if($avalues["Customer ID"]=="")
			$avalues["Customer ID"]=prepare_for_db("Customer ID",$_SESSION[$sessionPrefix."_masterkey1"]);
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Customer_Module_Assignment_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="dbo.Customers")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["Customer ID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
