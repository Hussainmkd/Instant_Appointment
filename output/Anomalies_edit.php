...
$layout->blocks["top"][] = "details";$page_layouts["Anomalies_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Anomalies_inline_edit.htm" : "Anomalies_edit.htm";
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Module ID"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Module ID"] = $value;
		
			}
	
		}
//	processing Module ID - end
//	processing Anomaly Description - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Anomaly Description"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Anomaly Description"] = $value;
		
			}
	
		}
//	processing Anomaly Description - end
//	processing Anomaly Type - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Anomaly Type"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Anomaly Type"] = $value;
		
			}
	
		}
//	processing Anomaly Type - end
//	processing Date Time - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Date Time"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Date Time"] = $value;
		
			}
	
		}
//	processing Date Time - end
//	processing Action Taken - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "Action Taken"=="Password" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["Action Taken"] = $value;
		
			}
	
		}
//	processing Action Taken - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Anomalies_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_Anomalies->Copy();
...
if(!$data)
{
	...
	{
		header("Location: Anomalies_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Module ID"] = $evalues["Module ID"];
	$data["Anomaly Description"] = $evalues["Anomaly Description"];
	$data["Anomaly Type"] = $evalues["Anomaly Type"];
	$data["Date Time"] = $evalues["Date Time"];
	$data["Action Taken"] = $evalues["Action Taken"];
}
...
//	Module ID - 
		...
		$value = "";
				$value = DisplayLookupWizard("Module ID",$data["Module ID"],$data,$keylink,MODE_LIST);
...
//	Anomaly Type - 
		...
		$value = "";
				$value = DisplayLookupWizard("Anomaly Type",$data["Anomaly Type"],$data,$keylink,MODE_LIST);
...
//	Action Taken - 
		...
		$value = "";
				$value = DisplayLookupWizard("Action Taken",$data["Action Taken"],$data,$keylink,MODE_LIST);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Anomalies_view.php?".$viewlink."'\"");
