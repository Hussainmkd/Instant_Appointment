...
$layout->blocks["top"][] = "details";$page_layouts["Customer_Module_Assignment_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Customer_Module_Assignment_inline_edit.htm" : "Customer_Module_Assignment_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Customer_Module_Assignment_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_Customer_Module_Assignment->Copy();
...
if(!$data)
{
	...
	{
		header("Location: Customer_Module_Assignment_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	Customer ID - 
		...
		$value = "";
				$value = DisplayLookupWizard("Customer ID",$data["Customer ID"],$data,$keylink,MODE_LIST);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.Customer Module Assignment";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Customer_Module_Assignment_view.php?".$viewlink."'\"");
