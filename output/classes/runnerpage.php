	...
	*/
	...
	function getPageTitle()
	{
		switch($this->pageType)
		{
			...
			case PAGE_SEARCH:
				return $this->strCaption.': '.mlang_message("ADVANCED_SEARCH");
			case PAGE_EXPORT:
				return mlang_message("EXPORT");
			case PAGE_IMPORT:
				return mlang_message("IMPORT");
			default:
	...
	function commonAssign() 
	{
		...
		if (isMobile())
		{
			...
			$this->xt->displayBrickHidden("cancelbutton_mobile");
		}
	}
	...
	function addCommonJs() 
	{
		...
		if($this->timepicker)
		{
			...
			$this->AddCSSFile("include/ui.timepickr");
			$this->AddCSSFile("include/ui.theme");
		}
...
//			$this->AddCSSFile("include/redefineStyle");
	...
	function isShowMenu()
	{
		foreach($this->menuTables as $menuTable) 
		{
			if($this->permis[$menuTable['tName']]['add'] || $this->permis[$menuTable['tName']]['search'])
				return true;
		}
		return false;
	}
	...
	{	
		$captchaHTML = '<div class="captcha_block">
			<object width="210" height="65" data="securitycode.swf?ext=php" type="application/x-shockwave-flash">
				...
				<div style="white-space: nowrap;">'.mlang_message("SEC_TYPETHECODE").':</div>
		...
		if(isset($this->isCaptchaOk) && !$this->isCaptchaOk)
			$captchaHTML .= '<div class="error">'.mlang_message("SEC_INVALID_CAPTCHA_CODE").'</div>';
		...
		if($this->isTableType == "report" && $this->reportGroupFields)
		{
			...
			{			
				if($this->arrGroupsPerPage[$i]!=-1)
					...
					$rpp.= "<option value=\"-1\" ".($this->pageSize == $this->arrGroupsPerPage[$i] ? "selected" : "").">".mlang_message("SHOW_ALL")."</option>";
			}	
		}	
		else{
				...
				for($i=0;$i<count($this->arrRecsPerPage);$i++)
				{
					if($this->arrRecsPerPage[$i]!=-1)
						...
						$rpp.= "<option value=\"-1\" ".($this->pageSize == $this->arrRecsPerPage[$i] ? "selected" : "").">".mlang_message("SHOW_ALL")."</option>";
				}
			}
		$rpp.="</select>";
