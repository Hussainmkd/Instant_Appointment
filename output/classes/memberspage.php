	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			foreach($this->groups as $idx => $g)
			{
				...
				if($_SESSION["UserID"] != $data["UserID"] || $g[0] != -1)
				{
			...
			//$row["begin"] = "<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["UserID"])."\">";
			
			$row["username"] = htmlspecialchars($data["UserID"]);
			$row["user"] = $data["UserID"];
			$usercheckbox_attrs.=" name=\"user_".count($this->users)
				."\" id=\"".count($this->users)."\"";
			
			$this->users[] = htmlspecialchars($data["UserID"]);
