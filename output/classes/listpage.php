...
class ListPage extends RunnerPage 
{
	...
	function commonAssign() 
	{
		...
		$this->xt->assign("record_controls", $this->permis[$this->tName]['add'] || $this->isDispGrid());
	...
	function fillGridShowInfo(&$rowInfoArr)
	{
		...
		if(($this->isUseInlineAdd || $this->showAddInPopup) && $this->permis[$this->tName]['add']) 
		{
			...
			for($i = 0; $i < count($this->allDetailsTablesArr); $i ++) 
			{
				...
				//	$record[$dShortTable."_dtablelink_attrs"].= " onmouseover=\"RollDetailsLink.showPopup(this,'".$dShortTable
				//	."_detailspreview.php'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
	...
	{		
		for($i=0;$i<count($this->listFields);$i++) 
		{
			...
			if($this->setAttrAlign($i))
				$span.= "class=\"".$this->setAttrAlign($i)."\" ";
