...
$layout->blocks["bare"][] = "mastergrid";$page_layouts["LU_Anomaly_Type_masterlist"] = $layout;


if($detailtable=="dbo.Anomalies")
{
		$where.= GetFullFieldName("Code")."=".make_db_value("Code",$keys[1-1]);
	$showKeys .= " ".GetFieldLabel("dbo_LU_Anomaly_Type","Code").": ".$keys[1-1];
...
//	Tamper Type - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Tamper Type", ""),"field=Tamper+Type".$keylink);
			$xt->assign("Tamper_Type_mastervalue",$value);
	$xt->display("LU_Anomaly_Type_masterlist.htm");
	$strTableName=$oldTableName;
}
