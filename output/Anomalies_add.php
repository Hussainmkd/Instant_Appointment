...
$layout->blocks["top"][] = "details";$page_layouts["Anomalies_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "Anomalies_inline_add.htm";
else
	$templatefile = "Anomalies_add.htm";
...
//	processing Anomaly Description - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_Anomaly_Description_".$id);
		$type=postvalue("type_Anomaly_Description_".$id);
		if (FieldSubmitted("Anomaly Description_".$id))
		{
				$value=prepare_for_db("Anomaly Description",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Anomaly Description"=="Password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Anomaly Description"]=$value;
		}
		}
//	processibng Anomaly Description - end
//	processing Anomaly Type - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_Anomaly_Type_".$id);
		$type=postvalue("type_Anomaly_Type_".$id);
		if (FieldSubmitted("Anomaly Type_".$id))
		{
				$value=prepare_for_db("Anomaly Type",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Anomaly Type"=="Password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Anomaly Type"]=$value;
		}
		}
//	processibng Anomaly Type - end
...
//	processing Action Taken - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_Action_Taken_".$id);
		$type=postvalue("type_Action_Taken_".$id);
		if (FieldSubmitted("Action Taken_".$id))
		{
				$value=prepare_for_db("Action Taken",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "Action Taken"=="Password" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["Action Taken"]=$value;
		}
		}
//	processibng Action Taken - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="dbo.Actions")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["Action Taken"]=="")
			$avalues["Action Taken"]=prepare_for_db("Action Taken",$_SESSION[$sessionPrefix."_masterkey1"]);
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="dbo.LU_Anomaly Type")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["Anomaly Type"]=="")
			$avalues["Anomaly Type"]=prepare_for_db("Anomaly Type",$_SESSION[$sessionPrefix."_masterkey1"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'Anomalies_edit.php?'.$keylink.'\'>'.mlang_message("EDIT").'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'Anomalies_view.php?'.$keylink.'\'>'.mlang_message("VIEW").'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: Anomalies_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="dbo.Actions")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["Action Taken"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="dbo.LU_Anomaly Type")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["Anomaly Type"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["Anomaly Description"]=@$avalues["Anomaly Description"];
	$defvalues["Anomaly Type"]=@$avalues["Anomaly Type"];
	$defvalues["Date Time"]=@$avalues["Date Time"];
	$defvalues["Action Taken"]=@$avalues["Action Taken"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("Anomaly_Description_label",true);
	if(isEnableSection508())
		$xt->assign_section("Anomaly_Description_label","<label for=\"".GetInputElementId("Anomaly Description", $id)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("Anomaly Type"))
		$xt->assign("Anomaly_Type_fieldblock",true);
	else
		$xt->assign("Anomaly_Type_tabfieldblock",true);
	$xt->assign("Anomaly_Type_label",true);
	if(isEnableSection508())
		$xt->assign_section("Anomaly_Type_label","<label for=\"".GetInputElementId("Anomaly Type", $id)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Action_Taken_label","<label for=\"".GetInputElementId("Action Taken", $id)."\">","</label>");
...
//	Anomaly Description - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Anomaly Description", ""),"field=Anomaly+Description".$keylink,"",MODE_LIST);
	$showValues["Anomaly Description"] = $value;
	$showFields[] = "Anomaly Description";
		$showRawValues["Anomaly Description"] = substr($data["Anomaly Description"],0,100);
	}	
//	Anomaly Type - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Anomaly Type", ""),"field=Anomaly+Type".$keylink,"",MODE_LIST);
	$showValues["Anomaly Type"] = $value;
	$showFields[] = "Anomaly Type";
		$showRawValues["Anomaly Type"] = substr($data["Anomaly Type"],0,100);
	}	
...
//	Action Taken - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"Action Taken", ""),"field=Action+Taken".$keylink,"",MODE_LIST);
	$showValues["Action Taken"] = $value;
	$showFields[] = "Action Taken";
		$showRawValues["Action Taken"] = substr($data["Action Taken"],0,100);
	}	
