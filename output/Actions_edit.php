...
include("include/Actions_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["Actions_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Actions_inline_edit.htm" : "Actions_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: Actions_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: Actions_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.Actions";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Actions_view.php?".$viewlink."'\"");
