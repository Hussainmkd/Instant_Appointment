...
$layout->blocks["top"][] = "export";$page_layouts["System_Users_export"] = $layout;
...
$xt->display("System_Users_export.htm");

function ExportToWord()
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-word");
	header("Content-Disposition: attachment;Filename=System_Users.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=System_Users.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$values = array();
			$values["UserID"] = GetData($row,"UserID","");
			$values["Password"] = GetData($row,"Password","");
			$values["User Name"] = GetData($row,"User Name","");
			$values["Role"] = DisplayLookupWizard("Role",$row["Role"],$row,"",MODE_EXPORT);		
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["UserID"] = GetData($row,"UserID",$format);
			$format="";
			$values["Password"] = GetData($row,"Password",$format);
			$format="";
			$values["User Name"] = GetData($row,"User Name",$format);
		$values["Role"] = DisplayLookupWizard("Role",$row["Role"],$row,"",MODE_EXPORT);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"','""',$values["User Name"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_System_Users","UserID")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_System_Users","Password")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_System_Users","User_Name")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_System_Users","Role")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("dbo_System_Users","UserID")."</td>";
		echo "<td>".GetFieldLabel("dbo_System_Users","Password")."</td>";
		echo "<td>".GetFieldLabel("dbo_System_Users","User_Name")."</td>";
		echo "<td>".GetFieldLabel("dbo_System_Users","Role")."</td>";
	}
	echo "</tr>";
		$totals["UserID"]=0;
		$totalsFields[]=array('fName'=>"UserID", 'totalsType'=>'', 'viewFormat'=>"");
		$totals["Password"]=0;
		$totalsFields[]=array('fName'=>"Password", 'totalsType'=>'', 'viewFormat'=>"");
		$totals["User Name"]=0;
		$totalsFields[]=array('fName'=>"User Name", 'totalsType'=>'', 'viewFormat'=>"");
		$totals["Role"]=0;
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["UserID"]);
					else
						echo htmlspecialchars($values["UserID"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["User Name"]);
					else
						echo htmlspecialchars($values["User Name"]);
			echo '</td>';
							echo '<td>';
