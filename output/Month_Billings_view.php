...
$layout->blocks["top"][] = "details";$page_layouts["Month_Billings_view"] = $layout;
...
//	show one record only
	$keys=array();
	$strWhereClause="";
	$keys["Bill ID"]=postvalue("editid1");
...
//Bill ID - 
	...
	$value="";
	$value = ProcessLargeText(GetData($data,"Bill ID", ""),"","",MODE_VIEW);
	if($mainTableOwnerID=="Bill ID")
		$ownerIdValue=$value;
	$xt->assign("Bill_ID_value",$value);
	if(!$pageObject->isAppearOnTabs("Bill ID"))
		$xt->assign("Bill_ID_fieldblock",true);
	else
		$xt->assign("Bill_ID_tabfieldblock",true);
////////////////////////////////////////////
//Customer ID - 
	
	$value="";
	$value = ProcessLargeText(GetData($data,"Customer ID", ""),"","",MODE_VIEW);
...
//Bill Month - 
	...
	$value="";
	$value = ProcessLargeText(GetData($data,"Bill Month", ""),"","",MODE_VIEW);
	if($mainTableOwnerID=="Bill Month")
		$ownerIdValue=$value;
	$xt->assign("Bill_Month_value",$value);
...
//Previous Readings - 
	...
	$value="";
	$value = ProcessLargeText(GetData($data,"Previous Readings", ""),"","",MODE_VIEW);
	if($mainTableOwnerID=="Previous Readings")
		$ownerIdValue=$value;
	$xt->assign("Previous_Readings_value",$value);
...
//Unit Consumed - 
	...
	$value="";
	$value = ProcessLargeText(GetData($data,"Unit Consumed", ""),"","",MODE_VIEW);
	if($mainTableOwnerID=="Unit Consumed")
		$ownerIdValue=$value;
	$xt->assign("Unit_Consumed_value",$value);
...
//Bill Amount - 
	...
	$value="";
	$value = ProcessLargeText(GetData($data,"Bill Amount", ""),"","",MODE_VIEW);
	if($mainTableOwnerID=="Bill Amount")
		$ownerIdValue=$value;
	$xt->assign("Bill_Amount_value",$value);
...
//Due Date - 
	...
	$value="";
	$value = ProcessLargeText(GetData($data,"Due Date", ""),"","",MODE_VIEW);
	if($mainTableOwnerID=="Due Date")
		$ownerIdValue=$value;
	$xt->assign("Due_Date_value",$value);
...
/*$jsKeysObj = 'window.recKeysObj = {';
	$jsKeysObj .= "'".jsreplace("Bill ID")."': '".(jsreplace(@$data["Bill ID"]))."', ";
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='Month_Billings_edit.php?".$editlink."'\"");
...
$templatefile = "Month_Billings_view.htm";
