...
$layout->blocks["top"][] = "details";$page_layouts["LU_Anomaly_Type_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "LU_Anomaly_Type_inline_add.htm";
else
	$templatefile = "LU_Anomaly_Type_add.htm";
...
//	processing Tamper Type - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["Tamper Type"]=$value;
		}
		}
//	processibng Tamper Type - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'LU_Anomaly_Type_edit.php?'.$keylink.'\'>'.mlang_message("EDIT").'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'LU_Anomaly_Type_view.php?'.$keylink.'\'>'.mlang_message("VIEW").'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: LU_Anomaly_Type_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["Code"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["Code"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["Tamper Type"]=@$avalues["Tamper Type"];
}
