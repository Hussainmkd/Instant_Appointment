...
$layout->blocks["top"][] = "details";$page_layouts["LU_Anomaly_Type_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "LU_Anomaly_Type_inline_edit.htm" : "LU_Anomaly_Type_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["Code"]=urldecode(postvalue("editid1"));
$savedKeys["Code"]=urldecode(postvalue("editid1"));
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["Code"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: LU_Anomaly_Type_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_LU_Anomaly_Type->Copy();
...
if(!$data)
{
	...
	{
		header("Location: LU_Anomaly_Type_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Tamper Type"] = $evalues["Tamper Type"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"Code", "")));
...
//	detail tables
	$showDetailKeys["dbo.Anomalies"]["masterkey1"] = $data["Code"];		
...
//	Code - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"Code", ""),"field=Code".$keylink,"",MODE_LIST);
...
//	Tamper Type - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"Tamper Type", ""),"field=Tamper+Type".$keylink,"",MODE_LIST);
	$showValues["Tamper Type"] = $value;
	$showFields[] = "Tamper Type";
		$showRawValues["Tamper Type"] = substr($data["Tamper Type"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.LU_Anomaly Type";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='LU_Anomaly_Type_view.php?".$viewlink."'\"");
