...
include("include/LU_Customer_Type_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["LU_Customer_Type_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "LU_Customer_Type_inline_edit.htm" : "LU_Customer_Type_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: LU_Customer_Type_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_LU_Customer_Type->Copy();
...
if(!$data)
{
	...
	{
		header("Location: LU_Customer_Type_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	detail tables
	$showDetailKeys["dbo.Customers"]["masterkey1"] = $data["Code"];		
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.LU_Customer Type";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='LU_Customer_Type_view.php?".$viewlink."'\"");
