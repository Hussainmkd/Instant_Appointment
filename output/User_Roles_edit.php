...
include("include/User_Roles_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["User_Roles_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "User_Roles_inline_edit.htm" : "User_Roles_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: User_Roles_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_User_Roles->Copy();
...
if(!$data)
{
	...
	{
		header("Location: User_Roles_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	detail tables
	$showDetailKeys["dbo.System Users"]["masterkey1"] = $data["Code"];		
...
//	Role - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"Role", ""),"field=Role".$keylink,"",MODE_LIST);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.User Roles";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='User_Roles_view.php?".$viewlink."'\"");
