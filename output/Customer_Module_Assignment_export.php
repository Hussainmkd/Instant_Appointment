...
$layout->blocks["top"][] = "export";$page_layouts["Customer_Module_Assignment_export"] = $layout;
...
$xt->display("Customer_Module_Assignment_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Customer_Module_Assignment.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$values["Customer ID"] = DisplayLookupWizard("Customer ID",$row["Customer ID"],$row,"",MODE_EXPORT);		
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Customer_Module_Assignment.csv");
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_Customer_Module_Assignment","ID")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_Customer_Module_Assignment","Customer_ID")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_Customer_Module_Assignment","Module_ID")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("dbo_Customer_Module_Assignment","ID")."</td>";
		echo "<td>".GetFieldLabel("dbo_Customer_Module_Assignment","Customer_ID")."</td>";
		echo "<td>".GetFieldLabel("dbo_Customer_Module_Assignment","Module_ID")."</td>";
	}
	echo "</tr>";
		$totals["ID"]=0;
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["Customer ID"]);
				else
