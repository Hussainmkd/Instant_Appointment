...
$layout->blocks["top"][] = "details";$page_layouts["Electricity_Rates_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "Electricity_Rates_inline_edit.htm" : "Electricity_Rates_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["Units"]=urldecode(postvalue("editid1"));
$savedKeys["Units"]=urldecode(postvalue("editid1"));
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["Units"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: Electricity_Rates_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_Electricity_Rates->Copy();
...
if(!$data)
{
	...
	{
		header("Location: Electricity_Rates_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["PerUnit Price"] = $evalues["PerUnit Price"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"Units", "")));
...
//	Units - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"Units", ""),"field=Units".$keylink,"",MODE_LIST);
	$showValues["Units"] = $value;
	$showFields[] = "Units";
		$showRawValues["Units"] = substr($data["Units"],0,100);

//	PerUnit Price - Number

		$value = "";
			$value = ProcessLargeText(GetData($data,"PerUnit Price", "Number"),"field=PerUnit+Price".$keylink,"",MODE_LIST);
	$showValues["PerUnit Price"] = $value;
	$showFields[] = "PerUnit Price";
		$showRawValues["PerUnit Price"] = substr($data["PerUnit Price"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "dbo.Electricity Rates";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='Electricity_Rates_view.php?".$viewlink."'\"");
