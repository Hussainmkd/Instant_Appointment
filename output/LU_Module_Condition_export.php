...
$layout->blocks["top"][] = "export";$page_layouts["LU_Module_Condition_export"] = $layout;
...
$xt->display("LU_Module_Condition_export.htm");

function ExportToWord()
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-word");
	header("Content-Disposition: attachment;Filename=LU_Module_Condition.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=LU_Module_Condition.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$values["Condition"] = GetData($row,"Condition","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=LU_Module_Condition.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["Condition"] = GetData($row,"Condition",$format);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"','""',$values["Condition"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_LU_Module_Condition","Code")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("dbo_LU_Module_Condition","Condition")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("dbo_LU_Module_Condition","Code")."</td>";
		echo "<td>".GetFieldLabel("dbo_LU_Module_Condition","Condition")."</td>";
	}
	echo "</tr>";
		$totals["Code"]=0;
		$totalsFields[]=array('fName'=>"Code", 'totalsType'=>'', 'viewFormat'=>"");
		$totals["Condition"]=0;
		$totalsFields[]=array('fName'=>"Condition", 'totalsType'=>'', 'viewFormat'=>"");
	$totals=array();
// write data rows
	$iNumberOfRows = 0;
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Condition"]);
					else
						echo htmlspecialchars($values["Condition"]);
			echo '</td>';
			echo "</tr>";
		}		
